...
type PPMDriverInfo struct {
	...
	InfPath   string `json:"infPath"`   // e.g. C:\Windows\INF\oem141.inf
	PpkgPath  string `json:"ppkgPath"`  // e.g. C:\Windows\provisioning\packages\XXX.ppkg
	SysPath   string `json:"sysPath"`   // e.g. C:\Windows\System32\drivers\XXX.sys
}
...
func GetPPMDrivers() []PPMDriverInfo {
	...
	for _, raw := range rawDrivers {
		infPath := ""
		if raw.InfName != "" {
			infPath = "C:\\Windows\\INF\\" + raw.InfName
		}
		...
		// Try to find .sys path from the .inf file
		sysPath := findSysPathFromInf(raw.InfName)
...
func getPPMProvisioningPath() string {
	...
	output, err := runPowerShellHidden(
		`Get-ChildItem -Path "C:\Windows\provisioning\packages" -Filter "*PPM*.ppkg" -ErrorAction SilentlyContinue | Select-Object -ExpandProperty FullName`)
	if err != nil {
		return ""
	}

	// Return first PPM .ppkg file found
...
// findSysPathFromInf extracts the .sys driver binary path from an .inf file
func findSysPathFromInf(infName string) string {
	if infName == "" {
		return ""
	}
	infFullPath := "C:\\Windows\\INF\\" + infName
	output, err := runPowerShellHidden(
		`$inf = Get-Content "` + infFullPath + `" -ErrorAction SilentlyContinue; ` +
		`if ($inf) { ` +
		`$sysFile = ($inf | Select-String '\.sys' | Select-Object -First 1).Line; ` +
		`if ($sysFile -match '([\w-]+\.sys)') { ` +
		`$matches[1] ` +
		`} }`)
	if err != nil || output == "" {
		return ""
	}
	sysName := strings.TrimSpace(output)
	if sysName == "" {
		return ""
	}
	// Check C:\Windows\System32\drivers\
	sysPath := "C:\\Windows\\System32\\drivers\\" + sysName
	checkOutput, err := runPowerShellHidden(
		"if (Test-Path '" + sysPath + "') { 'EXISTS' }")
	if err != nil || !strings.Contains(checkOutput, "EXISTS") {
		// Also check C:\Windows\System32\DriverStore\
		altOutput, _ := runPowerShellHidden(
			"Get-ChildItem 'C:\\Windows\\System32\\DriverStore' -Recurse -Filter '" + sysName + "' -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty FullName")
		altPath := strings.TrimSpace(altOutput)
		if altPath != "" {
			return altPath
		}
		return ""
