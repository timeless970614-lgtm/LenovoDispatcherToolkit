<template>
  <div class="func-check-page">
    ...
    <div v-if="activeTab === 'b'" class="func-content">
      ...
      <!-- NPU Parameter Reference (default expanded, above Smart Auto-Scheduler) -->
      <div class="card" v-if="!npuDriverError">
        <div class="card-header">
          <h3 class="card-title" style="font-size:13px"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg> Parameter Reference</h3>
      ...
      <div class="card" v-if="!npuDriverError">
        <div class="card-header">
          <h3 class="card-title"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg> Smart Auto-Scheduler</h3>
          <span v-if="npuSchedRunning" class="npu-sched-badge running">● Running</span>
      ...
      <div v-for="(dev, idx) in npuDeviceList" :key="dev.index" class="card npu-device-card">
        <div class="card-header"><h3 class="card-title"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="2" width="20" height="20" rx="4"/><path d="M9 9h6v6H9z"/></svg> NPU Device {{ dev.index }} - {{ dev.modelName || 'Unknown' }}</h3><span :class="['npu-dev-badge', dev.coreCount > 0 ? 'badge-online' : 'badge-offline']">{{ dev.coreCount > 0 ? 'ONLINE' : 'OFFLINE' }}</span></div>
        <div class="npu-props-grid">
          <div class="npu-prop-item"><span class="npu-prop-label">Vendor ID</span><span class="npu-prop-value mono">{{ dev.vendorId ? '0x' + dev.vendorId.toString(16).toUpperCase().padStart(4,'0') : 'N/A' }}</span></div>
          <div class="npu-prop-item"><span class="npu-prop-label">Serial Number</span><span class="npu-prop-value mono">{{ dev.serialNumber || 'N/A' }}</span></div>
          <div class="npu-prop-item"><span class="npu-prop-label">Computing Power</span><span class="npu-prop-value highlight">{{ dev.computingPower > 0 ? dev.computingPower + ' TOPS' : 'N/A' }}</span></div>
          <div class="npu-prop-item"><span class="npu-prop-label">Core Count</span><span class="npu-prop-value">{{ dev.coreCount > 0 ? dev.coreCount : 'N/A' }}</span></div>
          <div class="npu-prop-item"><span class="npu-prop-label">DDR Size</span><span class="npu-prop-value">{{ dev.ddrSizeMB > 0 ? dev.ddrSizeMB + ' MB' : 'N/A' }}</span></div>
          <div class="npu-prop-item"><span class="npu-prop-label">Firmware Ver</span><span class="npu-prop-value mono">{{ dev.firmwareVer || 'N/A' }}</span></div>
        </div>
        <div class="npu-metrics-grid">
          <div class="npu-metric-item"><div class="npu-metric-label">IPU Util</div><div class="npu-metric-bar-wrap"><div class="npu-metric-bar" :style="{ width: (dev.ipuUtiliRate * 100) + '%', background: utilColor(dev.ipuUtiliRate) }"></div></div><div class="npu-metric-value" :style="{ color: utilColor(dev.ipuUtiliRate) }">{{ (dev.ipuUtiliRate * 100).toFixed(1) }}%</div></div>
          <div class="npu-metric-item"><div class="npu-metric-label">IPU Freq</div><div class="npu-metric-value highlight">{{ dev.ipuFrequencyHz > 0 ? (dev.ipuFrequencyHz / 1e6).toFixed(2) + ' MHz' : 'N/A' }}</div></div>
          <div class="npu-metric-item"><div class="npu-metric-label">Voltage</div><div class="npu-metric-value">{{ dev.ipuVoltageMV > 0 ? dev.ipuVoltageMV.toFixed(1) + ' mV' : 'N/A' }}</div></div>
          <div class="npu-metric-item"><div class="npu-metric-label">Board Power</div><div class="npu-metric-value highlight">{{ dev.boardPowerW > 0 ? dev.boardPowerW.toFixed(2) + ' W' : 'N/A' }}</div></div>
          <div class="npu-metric-item"><div class="npu-metric-label">Temperature</div><div class="npu-metric-value" :style="{ color: tempColor(dev.temperatureC) }">{{ dev.temperatureC > 0 ? dev.temperatureC.toFixed(1) + ' C' : 'N/A' }}</div></div>
          <div class="npu-metric-item"><div class="npu-metric-label">Memory</div><div class="npu-metric-value">{{ dev.memUsedMB || 0 }}/{{ dev.memTotalMB || 0 }} MB</div></div>
        </div>
        <div class="npu-dvfs-section">
          <div class="npu-section-title">DVFS &amp; Power Management</div>
          <div class="npu-dvfs-row"><span class="npu-dvfs-label">Mode:</span><span :class="['npu-dvfs-badge', dev.dvfsMode === 'PERFORMANCE' ? 'badge-perf' : dev.dvfsMode === 'ONDEMAND' ? 'badge-ondemand' : dev.dvfsMode === 'POWERLIMIT' ? 'badge-powerlimit' : 'badge-na']">{{ dev.dvfsMode || 'N/A' }}</span><div class="npu-dvfs-btns"><button class="btn btn-npu btn-npu-perf" @click="setNPUDVFS(dev.index, 'PERFORMANCE')" :disabled="npuSettingDVFS === dev.index"><span v-if="npuSettingDVFS === dev.index" class="spinner-small"></span><span v-else>PERF</span></button><button class="btn btn-npu btn-npu-ondemand" @click="setNPUDVFS(dev.index, 'ONDEMAND')" :disabled="npuSettingDVFS === dev.index"><span v-if="npuSettingDVFS === dev.index" class="spinner-small"></span><span v-else>ONDEMAND</span></button><button class="btn btn-npu btn-npu-powerlimit" @click="setNPUDVFS(dev.index, 'POWERLIMIT')" :disabled="npuSettingDVFS === dev.index"><span v-if="npuSettingDVFS === dev.index" class="spinner-small"></span><span v-else>PWR-LIM</span></button></div><span v-if="dev.dvfsResult" :class="['npu-dvfs-result', dev.dvfsResult.startsWith('Error') ? 'result-error' : 'result-ok']">{{ dev.dvfsResult }}</span></div>
          <div v-if="npuPowerStatus[dev.index]" class="npu-power-metrics-row">
            <div class="npu-power-metric"><span class="npu-pm-label">IPU Freq</span><span class="npu-pm-val">{{ npuPowerStatus[dev.index].curIpuFreqMhz > 0 ? npuPowerStatus[dev.index].curIpuFreqMhz.toFixed(0) + ' MHz' : 'N/A' }}</span></div>
            <div class="npu-power-metric"><span class="npu-pm-label">Board Power</span><span class="npu-pm-val">{{ npuPowerStatus[dev.index].boardPowerW > 0 ? npuPowerStatus[dev.index].boardPowerW.toFixed(2) + ' W' : 'N/A' }}</span></div>
            <div class="npu-power-metric"><span class="npu-pm-label">IPU Load</span><span class="npu-pm-val">{{ npuPowerStatus[dev.index].ipuLoadPct >= 0 ? npuPowerStatus[dev.index].ipuLoadPct.toFixed(1) + '%' : 'N/A' }}</span></div>
            <div class="npu-power-metric"><span class="npu-pm-label">Voltage</span><span class="npu-pm-val">{{ npuPowerStatus[dev.index].coreVoltageMV > 0 ? npuPowerStatus[dev.index].coreVoltageMV.toFixed(0) + ' mV' : 'N/A' }}</span></div>
            <div class="npu-power-metric" v-if="npuPowerStatus[dev.index].core0TempC"><span class="npu-pm-label">Core0</span><span class="npu-pm-val" :style="{ color: npuPowerStatus[dev.index].core0TempC >= 80 ? '#ef4444' : npuPowerStatus[dev.index].core0TempC >= 60 ? '#f59e0b' : '#22c55e' }">{{ npuPowerStatus[dev.index].core0TempC.toFixed(1) }}C</span></div>
            <div class="npu-power-metric" v-if="npuPowerStatus[dev.index].core1TempC"><span class="npu-pm-label">Core1</span><span class="npu-pm-val" :style="{ color: npuPowerStatus[dev.index].core1TempC >= 80 ? '#ef4444' : npuPowerStatus[dev.index].core1TempC >= 60 ? '#f59e0b' : '#22c55e' }">{{ npuPowerStatus[dev.index].core1TempC.toFixed(1) }}C</span></div>
          </div>
          <div class="npu-clock-lock-section">
            <div class="npu-section-subtitle">Clock Lock (700-1400 MHz)</div>
            <div class="npu-clock-lock-row"><div class="npu-cl-input-group"><label>Max</label><input type="number" class="npu-cl-input" :value="(npuClockLocking[dev.index] || (npuClockLocking[dev.index] = {maxMhz:1400,minMhz:700})).maxMhz" @input="(e) => { if(!npuClockLocking[dev.index]) npuClockLocking[dev.index] = {maxMhz:1400,minMhz:700}; npuClockLocking[dev.index].maxMhz = parseInt(e.target.value) || 1400 }" min="700" max="1400" /></div><div class="npu-cl-sep">-</div><div class="npu-cl-input-group"><label>Min</label><input type="number" class="npu-cl-input" :value="(npuClockLocking[dev.index] || (npuClockLocking[dev.index] = {maxMhz:1400,minMhz:700})).minMhz" @input="(e) => { if(!npuClockLocking[dev.index]) npuClockLocking[dev.index] = {maxMhz:1400,minMhz:700}; npuClockLocking[dev.index].minMhz = parseInt(e.target.value) || 700 }" min="700" max="1400" /></div><div class="npu-cl-presets"><button class="npu-preset-btn" @click="npuClockLocking[dev.index] = {maxMhz:1400,minMhz:700}">Full</button><button class="npu-preset-btn" @click="npuClockLocking[dev.index] = {maxMhz:1200,minMhz:700}">1200</button><button class="npu-preset-btn" @click="npuClockLocking[dev.index] = {maxMhz:1000,minMhz:700}">1000</button></div><button class="btn btn-npu btn-npu-clockset" @click="setNPUClockLock(dev.index)">Apply</button></div>
            <div v-if="npuPowerStatus[dev.index]" class="npu-cl-info">Current: {{ npuPowerStatus[dev.index].lockIpuMaxMhz > 0 ? npuPowerStatus[dev.index].lockIpuMaxMhz.toFixed(0) : '?' }} MHz - {{ npuPowerStatus[dev.index].lockIpuMinMhz > 0 ? npuPowerStatus[dev.index].lockIpuMinMhz.toFixed(0) : '?' }} MHz</div>
            <span v-if="npuClockLocking[dev.index] && npuClockLocking[dev.index].result" :class="['npu-dvfs-result', npuClockLocking[dev.index].result.Success ? 'result-ok' : 'result-error']">{{ npuClockLocking[dev.index].result.Message }}</span>
          </div>
          <div class="npu-reset-row"><button class="btn btn-npu btn-npu-reset" @click="resetNPUDefaults(dev.index)">Reset to Defaults (PERFORMANCE, 700-1400 MHz)</button></div>
          <span v-if="npuPowerResult[dev.index]" :class="['npu-dvfs-result', npuPowerResult[dev.index].Success ? 'result-ok' : 'result-error']">{{ npuPowerResult[dev.index].Message }}</span>
        </div>
...
import { npuMethods } from '../npu-power-methods'
...
export default {
  ...
  methods: {
    ...npuMethods,
      ...
      const devIdx = this.npuSchedDev
      const settings = { ...this.npuSchedSettings }
      App.StartNPUScheduler(devIdx, JSON.stringify(settings)).then(r => {
        this.npuSchedStarting = false
        if (r && r.Success) {
          this.npuSchedRunning = true
          this.npuSchedState = { curMode: '', decision: 'Scheduler started', curUtilPct: -1, curTempC: 0, curPowerW: 0, curFreqMHz: 0, curLockMaxMHz: 0, curLockMinMHz: 0, lastSwitch: '' }
    ...
    stopNpuScheduler() {
      App.StopNPUScheduler().then(r => {
        ...
        this.npuSchedState = { curMode: '', decision: '', curUtilPct: -1, curTempC: 0, curPowerW: 0, curFreqMHz: 0, curLockMaxMHz: 0, curLockMinMHz: 0, lastSwitch: '' }
      })
    },
    pollNpuSchedulerState() {
      if (!this.npuSchedRunning) return
      App.GetNPUSchedulerState().then(r => {
        if (r && r.Success) {
          this.npuSchedState = { curMode: r.Mode || '', decision: r.Decision || '', curUtilPct: r.UtilPct || -1, curTempC: r.TempC || 0, curPowerW: r.PowerW || 0, curFreqMHz: r.FreqMHz || 0, curLockMaxMHz: r.LockMaxMHz || 0, curLockMinMHz: r.LockMinMHz || 0, lastSwitch: r.LastSwitch || '—' }
          setTimeout(() => this.pollNpuSchedulerState(), this.npuSchedSettings.checkSec * 1000)
        }
      }).catch(() => setTimeout(() => this.pollNpuSchedulerState(), 3000))
    },
...
/* NPU Power */
.npu-overview-grid{display:grid;grid-template-columns:auto repeat(3,1fr);gap:12px}
.npu-overview-item{background:var(--bg-tertiary);border-radius:8px;padding:12px 16px}
.npu-dev-count{grid-row:span 2;display:flex;flex-direction:column;align-items:center;justify-content:center}
.npu-dev-count-num{font-size:40px;font-weight:800;color:var(--lenovo-red);line-height:1}
.npu-dev-count-label{font-size:11px;color:var(--text-secondary);margin-top:4px}
.npu-info-label{font-size:11px;color:var(--text-secondary);margin-bottom:4px;text-transform:uppercase;letter-spacing:.05em}
.npu-info-value{font-size:14px;font-weight:600;color:var(--text-primary)}
.npu-info-value.mono{font-family:Consolas,Monaco,monospace;font-size:12px}
.npu-error-banner{display:flex;gap:12px;padding:12px;background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.3);border-radius:8px;margin-bottom:12px}
.error-icon{font-size:24px}
.error-content p{margin:0 0 4px;font-size:13px}
.error-detail{font-size:11px;color:var(--text-secondary);margin:2px 0;font-family:monospace}
.error-hint{font-size:11px;color:var(--text-tertiary);margin:0}
.npu-device-card{margin-bottom:16px}
.npu-props-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:14px}
.npu-prop-item{background:var(--bg-tertiary);border-radius:6px;padding:8px 10px;display:flex;flex-direction:column;gap:2px}
.npu-prop-label{font-size:10px;color:var(--text-secondary);text-transform:uppercase;letter-spacing:.04em}
.npu-prop-value{font-size:13px;font-weight:600;color:var(--text-primary)}
.npu-prop-value.highlight{color:var(--lenovo-red)}
.npu-prop-value.mono{font-family:Consolas,Monaco,monospace;font-size:12px}
.npu-metrics-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:8px;margin-bottom:14px}
.npu-metric-item{display:flex;flex-direction:column;gap:4px;background:var(--bg-tertiary);padding:10px 12px;border-radius:8px}
.npu-metric-label{font-size:10px;color:var(--text-secondary);text-transform:uppercase;letter-spacing:.04em}
.npu-metric-value{font-size:15px;font-weight:700;color:var(--text-primary)}
.npu-metric-value.highlight{color:var(--lenovo-red)}
.npu-metric-bar-wrap{height:4px;background:rgba(255,255,255,.08);border-radius:2px;overflow:hidden}
.npu-metric-bar{height:100%;border-radius:2px;transition:width .5s ease,background .5s ease}
.npu-dvfs-section{border-top:1px solid var(--border-color);padding-top:12px;margin-top:4px}
.npu-section-title{font-size:12px;font-weight:600;color:var(--text-primary);margin-bottom:8px;text-transform:uppercase;letter-spacing:.05em}
.npu-dvfs-row{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:10px}
.npu-dvfs-label{font-size:12px;color:var(--text-secondary)}
.npu-dvfs-badge{padding:3px 12px;border-radius:12px;font-size:11px;font-weight:700}
.badge-perf{background:rgba(239,68,68,.15);color:#ef4444}
.badge-ondemand{background:rgba(76,175,80,.15);color:#4caf50}
.badge-powerlimit{background:rgba(255,193,7,.15);color:#ffc107}
.badge-na{background:var(--bg-tertiary);color:var(--text-secondary)}
.badge-online{background:rgba(34,197,94,.15);color:#22c55e}
.badge-offline{background:rgba(239,68,68,.15);color:#ef4444}
.npu-dev-badge{padding:2px 8px;border-radius:6px;font-size:10px;font-weight:700}
.npu-dvfs-btns{display:flex;gap:5px}
.npu-dvfs-result{font-size:11px;padding:3px 8px;border-radius:4px}
.result-ok{background:rgba(34,197,94,.12);color:#22c55e}
.result-error{background:rgba(239,68,68,.12);color:#ef4444}
.npu-ctc-row{display:flex;gap:8px;align-items:center;margin-top:8px;font-size:12px}
.npu-ctc-label{color:var(--text-secondary)}
.npu-ctc-val{font-family:monospace;font-size:11px;color:var(--text-primary)}
.npu-power-metrics-row{display:flex;gap:6px;margin:10px 0;flex-wrap:wrap}
.npu-power-metric{display:flex;flex-direction:column;gap:2px;background:var(--bg-tertiary);border-radius:6px;padding:6px 10px;min-width:80px;text-align:center}
.npu-pm-label{font-size:10px;color:var(--text-tertiary)}
.npu-pm-val{font-size:13px;font-weight:600;color:var(--text-primary)}
.npu-clock-lock-section{margin-top:8px}
.npu-section-subtitle{font-size:11px;font-weight:600;color:var(--text-secondary);margin-bottom:6px;text-transform:uppercase;letter-spacing:.05em}
.npu-clock-lock-row{display:flex;align-items:flex-end;gap:6px;flex-wrap:wrap}
.npu-cl-input-group{display:flex;flex-direction:column;gap:2px}
.npu-cl-input-group label{font-size:10px;color:var(--text-secondary)}
.npu-cl-input{width:72px;padding:5px 8px;background:var(--bg-tertiary);border:1px solid var(--border-color);border-radius:6px;color:var(--text-primary);font-size:13px;font-family:inherit}
.npu-cl-input:focus{outline:none;border-color:var(--lenovo-red)}
.npu-cl-sep{padding-bottom:6px;color:var(--text-tertiary);font-size:14px}
.npu-cl-presets{display:flex;gap:4px;flex-wrap:wrap;padding-bottom:4px}
.npu-preset-btn{padding:3px 8px;font-size:10px;border-radius:4px;background:var(--bg-tertiary);border:1px solid var(--border-color);color:var(--text-secondary);cursor:pointer;transition:var(--transition)}
.npu-preset-btn:hover{border-color:var(--lenovo-red);color:var(--lenovo-red)}
.npu-cl-info{font-size:10px;color:var(--text-tertiary);margin-top:4px}
.btn-npu{display:inline-flex;align-items:center;gap:4px;padding:5px 10px;border-radius:6px;border:1px solid;font-size:11px;font-weight:600;cursor:pointer;transition:var(--transition);font-family:inherit}
.btn-npu-perf{background:rgba(239,68,68,.15);color:#ef4444;border-color:#ef4444}
.btn-npu-ondemand{background:rgba(76,175,80,.15);color:#4caf50;border-color:#4caf50}
.btn-npu-powerlimit{background:rgba(255,193,7,.15);color:#ffc107;border-color:#ffc107}
.btn-npu-perf:hover:not(:disabled){background:rgba(239,68,68,.3)}
.btn-npu-ondemand:hover:not(:disabled){background:rgba(76,175,80,.3)}
.btn-npu-powerlimit:hover:not(:disabled){background:rgba(255,193,7,.3)}
.btn-npu-clockset{font-size:11px;padding:5px 12px;background:rgba(33,150,243,.15);color:#2196f3;border-color:#2196f3}
