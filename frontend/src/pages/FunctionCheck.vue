    ...
    <div v-if="activeTab === 'gpu'" class="func-content">
        ...
        <div class="igpu-status">
          <div class="status-grid-2x4">
            ...
            <div class="status-item status-item-span-rows">
              <span class="status-label no-transform">Current Status <span class="live-dot"></span></span>
      ...
      <div class="card">
        ...
        <div class="diag-buttons">
          <button class="btn btn-primary btn-sm" @click="setIGPUMode(0)" :disabled="settingMode || igpuStatus.mode === 0">
            Enable DGPU (Mode 0)
          </button>
          <button class="btn btn-warning btn-sm" @click="setIGPUMode(1)" :disabled="settingMode || igpuStatus.mode === 1">
            Enable UMA Only (Mode 1)
          </button>
          <div v-if="settingMode" class="spinner-small"></div>
          <span v-if="settingResult" :class="['result-text', settingResult.success ? 'success' : 'error']">
        ...
        <div class="ppm-container">
          <div class="ppm-header">
            ...
            <div class="ppm-actions">
              <button class="btn btn-outline btn-sm" @click="powerMinimized = !powerMinimized" :title="powerMinimized ? 'Expand' : 'Minimize'">
                {{ powerMinimized ? '⤢' : '⤡' }}
              </button>
          ...
          <!-- Minimized View: PL1, PL2, Package Power only -->
          <div v-if="sysPower && powerMinimized" class="power-mini-bar">
            <span class="power-mini-item">
              <span class="power-mini-label">PKG</span>
              <span class="power-mini-value">{{ sysPower.cpuPowerWatts > 0 ? sysPower.cpuPowerWatts.toFixed(1) + 'W' : '-' }}</span>
            </span>
            <span class="power-mini-divider">|</span>
            <span class="power-mini-item">
              <span class="power-mini-label">PL1</span>
              <span class="power-mini-value">{{ sysPower.pl1Watts > 0 ? sysPower.pl1Watts.toFixed(1) + 'W' : '-' }}</span>
            </span>
            <span class="power-mini-divider">|</span>
            <span class="power-mini-item">
              <span class="power-mini-label">PL2</span>
              <span class="power-mini-value">{{ sysPower.pl2Watts > 0 ? sysPower.pl2Watts.toFixed(1) + 'W' : '-' }}</span>
            </span>
          </div>

          <div v-if="sysPower && !powerMinimized" class="ppm-grid power-inline-grid">
            <!-- CPU Power -->
...
export default {
  ...
  methods: {
    ...
    vantageGPULabel(val) {
      switch(val) {
        case 1: return 'IGPU priority mode '
        case 2: return 'Auto iGPU on battery'
        case 3: return 'Hybrid graphic mode'
...
.power-inline-grid .ppm-section:nth-child(1)::before { background: linear-gradient(90deg, var(--lenovo-red), var(--lenovo-red-light)); }
.power-inline-grid .ppm-section:nth-child(2)::before { background: linear-gradient(90deg, #f59e0b, #ef4444); }
.power-inline-grid .ppm-section:nth-child(3)::before { background: linear-gradient(90deg, var(--lenovo-red-dark), var(--lenovo-red)); }
.power-inline-grid .ppm-section:nth-child(4)::before { background: linear-gradient(90deg, var(--lenovo-red), var(--accent-yellow)); }
...
.ppm-section:hover {
  ...
  box-shadow: 0 4px 16px rgba(230, 63, 50, 0.08);
...
.ppm-card:hover {
  ...
  box-shadow: 0 2px 12px rgba(230, 63, 50, 0.08);
  transform: translateY(-1px);
}
...
.power-highlight {
  font-size: 1.15em;
  font-weight: 800;
  background: linear-gradient(135deg, var(--lenovo-red), var(--lenovo-red-light));
